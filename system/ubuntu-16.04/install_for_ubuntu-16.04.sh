...
sudo apt-get remove -y --purge unity-webapps-common
...
sudo add-apt-repository -yu ppa:otto-kesselgulasch/gimp
sudo apt-get update
sudo apt-get install -y gimp 
sudo add-apt-repository -yu ppa:samoilov-lex/aftl-stable 
sudo apt-get update 
...
sudo add-apt-repository -yu ppa:noobslab/themes  
sudo apt-get update  
sudo apt-get install -y flatabulous-theme
sudo add-apt-repository -yu ppa:noobslab/icons  
sudo apt-get update  
...
sudo apt-get remove -y quadrapassel Mahjongg sudoku aisleriot gbrainy mins
echo '\033[33m====================完成======================\033[32m'

#INSTALL DEB PACKAGES FOR UBUNTU
cd ..
cd ..
sh package/install_deb_ubuntu-16.04.sh
